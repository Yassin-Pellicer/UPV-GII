...
%token READ_ PRINT_ IF_ ELSE_ FOR_ RET_ IN_ RANGE_ WHILE_ USING_ TIMES_ DO_
%token INT_ BOOL_ SCOL_ COMMA_ SWITCH_ LESS_ EQUAL_ GREATER_ COL_ WHEN_
...
%type <aux> instSelec instIter instruccionUsingTimes instSwitch instWhen clause lClause
...
listParamForm 
    ...
    | tipoSimp ID_ COMMA_ listParamForm
    {
        $$.ref = insTdD($4.ref, $1);
...
instIter 
    ...
    | FOR_ ID_ IN_ RANGE_ PARA_ expre 
    {
        ...
        emite(EASIG, crArgPos(niv, $6.pos), crArgNul(), crArgPos(simb.n, simb.d));
    } 
    COMMA_ expre 
    {
        SIMB simb = obtTdS($2);
        ...
        $<aux>$.ref2 = creaLans(si);
        emite(EMAYEQ, crArgPos(simb.n, simb.d), crArgPos(niv, $9.pos), crArgEtq(-1));
    }
    PARC_ inst
    {
        SIMB simb = obtTdS($2);
        emite(ESUM, crArgPos(simb.n, simb.d), crArgEnt(1), crArgPos(simb.n, simb.d));
        emite(GOTOS, crArgNul(), crArgNul(), crArgEtq($<aux>10.ref1)); 
        completaLans($<aux>10.ref2, crArgEtq(si));
    }
    | WHILE_ PARA_ 
    {
        $<aux>$.ref2 = si;
    ...
    inst
    {
        emite(GOTOS, crArgNul(), crArgNul(), crArgEtq($<aux>3.ref2));
...
instruccionUsingTimes
    ...
    TIMES_ DO_ inst
    {
        ...
        int ref = creaLans(si);
        emite(EMAYEQ,  crArgPos(simb.n, simb.d), crArgPos(niv, $3.pos), crArgEtq(-1));
        emite(ESUM, crArgPos(simb.n, simb.d), crArgEnt(1), crArgPos(simb.n, simb.d));
        emite(GOTOS, crArgNul(), crArgNul(), crArgEtq($<aux>4.ref0));
        completaLans(ref, crArgEtq(si));
    }
    ;

instSwitch 
    : SWITCH_ ID_ LLAA_ LESS_
    {
        SIMB simb = obtTdS($2);
        if (simb.t != T_ENTERO)
    ...
    inst EQUAL_ 
    {
        $<aux>$.ref3 = creaLans(si);
        emite(GOTOS, crArgNul(), crArgNul(), crArgEtq(-1));
        completaLans($<aux>5.ref1, crArgEtq(si));
    }
    inst GREATER_ 
    {
        $<aux>$.ref4 = creaLans(si);
        emite(GOTOS, crArgNul(), crArgNul(), crArgEtq(-1));
        completaLans($<aux>5.ref2, crArgEtq(si));
    }
    inst LLAC_
    {
        completaLans($<aux>8.ref3, crArgEtq(si));   
        completaLans($<aux>11.ref4, crArgEtq(si));   
    }
    ;
...
clause
    : expre COL_ 
    {
        if($1.t != T_LOGICO && $1.t != T_ERROR)
            yyerror("El tipo del when debe ser lógico.");
        $<aux>$.ref0 = creaLans(si);
        emite(EIGUAL,  crArgPos(simb.n, simb.d), crArgEnt(0), crArgEtq(-1)); 
    } 
    inst
    {   
        $<aux>$.ref1 = creaLans(si);
        emite(GOTOS, crArgNul(), crArgNul(), crArgEtq(-1));
        completaLans($<aux>3.ref0, crArgEtq(si));
    }
    ;
