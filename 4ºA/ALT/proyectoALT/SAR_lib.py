...
import pickle
from spellsuggester import SpellSuggester  # Importa SpellSuggester desde su archivo
...
class SAR_Indexer:
    ...
    def set_spelling(self, use_spelling: bool, distance: str = None, threshold: int = None):
        """
        Activa o desactiva el corrector ortográfico en el índice.
        
        input: 
            - use_spelling (bool): activa o desactiva el corrector ortográfico.
            - distance (str): nombre de la función de distancia (por ejemplo, "levenshtein").
            - threshold (int): nivel de tolerancia de distancia.
        """
        self.use_spelling = use_spelling  # Activa o desactiva la corrección ortográfica
        if use_spelling:
            # Verificamos que la función de distancia solicitada esté en opcionesSpell
            if distance in opcionesSpell:
                # Creamos el corrector ortográfico con las palabras indexadas y los parámetros dados
                self.speller = SpellSuggester(
                    dist_functions=opcionesSpell,         # Pasamos el diccionario completo de funciones de distancia
                    vocab=list(self.index['all'].keys()), # Pasamos las claves del índice como vocabulario
                    default_distance=distance,            # Configura el tipo de distancia por defecto
                    default_threshold=threshold           # Configura el umbral de distancia por defecto
                )
            else:
                raise ValueError(f"La función de distancia '{distance}' no está definida en opcionesSpell.")
        else:
            self.speller = None  # Si no se usa corrección, resetea el speller
    ...
    def get_posting(self, term: str, field: Optional[str] = None):
        ...
        if '"' in term:
            ...
            # Recuperar el campo correspondiente o usar "all" por defecto
            field = field or "all"
            posting_list = self.index[field].get(term)
            
            # Si el término no está en el índice y la corrección ortográfica está activada
            if posting_list is None and self.use_spelling:
                # Obtener términos sugeridos usando el corrector
                suggested_terms = self.speller.suggest(term)
                
                # Si hay términos sugeridos, combinamos sus posting lists
                if suggested_terms:
                    posting_list = []
                    for suggested_term in suggested_terms:
                        term_posting = self.index[field].get(suggested_term, [])
                        posting_list = self.or_posting(posting_list, term_posting)
            
            return posting_list or []
