...
\fancypagestyle{plain}{
  \fancyhf{}
  \fancyfoot[L]{\footnotesize SHELL Y ENTORNOS LINUX}
  \fancyfoot[C]{\footnotesize MIABD 2026/2027}
...
\title{\Huge Análisis y evolución de las empresas del S\&P 500\\[0.5\baselineskip] \large Memoria del proyecto de Shell y Entornos Linux}
...
Estos scripts se almacenan en la ruta \texttt{project/scripts/} del proyecto.
...
La conservación del histórico de archivos durante 30 días permite comparar e interpretar la evolución de la economía durante un período corto y significativo sin generar reportes excesivamente largos.
...
El script es a prueba de fallos de conexión desde el cliente. Es decir, si por algún motivo el equipo pierde la red, el programa reintentará la descarga hasta 10 veces en un plazo de 6 minutos. También controla las descargas vacías o la ausencia del recurso a descargar. Los datasets se almacenan en la ruta \texttt{project/datasets} del proyecto.
...
Por otro lado, el script \texttt{script\_analisis\_csv\_html.sh} genera un informe en formato HTML compuesto por los siguientes archivos\footnote{Los archivos \texttt{script.js} y \texttt{style.css} se incluyen junto al informe porque están pensados para enriquecer \texttt{report\_<date>.html} con funcionalidades interactivas y estilos visuales, respectivamente.}:
...
Se ha preferido separar los reportes de texto en 4 ficheros distintos para hacer la lectura y consulta de la información más sencilla, ya que cada reporte tiene un número de columnas variable y longitudes distintas.
...
\item \texttt{cut} elimina el campo auxiliar donde se calculó esta distancia.
\end{enumerate}

\subsection{Reporte de evolución del mercado}

Este reporte muestra la evolución histórica del precio y de la capitalización bursátil de las empresas ordenados por capitalización media durante el período:

\begin{enumerate}[noitemsep]
\item Se recorren los datasets disponibles hasta la fecha seleccionada y mediante \texttt{awk} se vuelca cada registro por día en la salida.
...
\item \texttt{sort} ordena la entrada por prioridad, primero por capitalizacion media, (para conseguir una ordenación), luego por \texttt{Symbol}, y por último por fecha.
\item Un último \texttt{awk} agrupa las empresas mostrando su evolución.
\end{enumerate}

\subsection{Volcado de resultados a los informes}

El volcado de la información generada en los informes se hace de manera especial en ambos scripts de análisis.

\subsubsection{Reportes .txt}

En estos reportes la información se estructura tabularmente y se adapta al formato .txt mediante columnas y cabeceras formadas por caracteres ASCII para facilitar la lectura.

\begin{center}
\begin{minipage}{0.7575\textwidth}
\lstset{
    basicstyle=\ttfamily\small,
    keywordstyle=\color{blue},
    commentstyle=\color{gray},
...
+=================================================================+
| Date: 13/09/2026 | Processed entries: 485                       |
+=================================================================+
| Semiconductors                                                  |
+=================================================================+
| Symbol   | Company                         | Market Cap         |
+----------+---------------------------------+--------------------+
| NVDA     | Nvidia                          |  5,271,048,421,376 |
| AVGO     | Broadcom                        |  1,728,006,324,224 |
| MU       | Micron Technology               |  1,101,452,017,664 |
| AMD      | Advanced Micro Devices          |    842,569,351,168 |
\end{lstlisting}
\end{minipage}
\end{center}
\subsubsection{Reportes .html}
En estos reportes \texttt{script\_analisis\_csv\_html.sh} prepara la estructura HTML que sustituir en el \texttt{template.html} mediante el comando \texttt{sed}, y mediante el cual construir el informe final. 
\begin{center}
\begin{minipage}{0.7575\textwidth}
\lstset{
    basicstyle=\ttfamily\small,
...
El archivo \texttt{log.txt} reúne los mensajes de las ejecuciones.
Se registran el inicio y fin de las etapas, los archivos utilizados,
los reintentos de descarga y las filas descartadas durante el saneamiento.
La función \texttt{log} añade fecha y hora con milisegundos; el operador
\texttt{>>} conserva las entradas anteriores:
...
Los mensajes de detalle incluyen la hora y etiquetas como
\texttt{[ok]} para operaciones correctas y \texttt{[ko]} para fallos.
La funcion \texttt{run\_step} comprueba el estado de salida de cada
script: si falla, registra la etapa afectada y termina la ejecución
con \texttt{exit 1}.

Para comandos como \texttt{sort}, se redirige la salida de error
(descriptor 2) a una función que registra cada mensaje junto al
nombre del comando:
...
# Fragmento de una tuberia:
sort -t',' -k10,10nr 2> >(log_command_error "sort") |
\end{lstlisting}

Este mecanismo permite localizar errores y conocer cuándo ocurrieron.

\section{Conclusión}

\begingroup

Este proyecto permite seguir la evolución de las empresas del S\&P 500 y convertir datos financieros dispersos en información organizada y fácil de consultar. Su propósito es facilitar la comparación entre empresas y sectores, identificar cuáles concentran mayor capitalización y observar cómo cambian sus precios a lo largo del tiempo.

En lo formativo se ha podido demostrar la utilidad de comandos de sellscript aplicados a la extracción de patrones comunes provenientes de una fuente de datos de manera automática mediante \texttt{awk, grep, sort, tail, sec}, y utilidades de automatización como \texttt{cron}.

El proyecto deja frentes abiertos y margen de mejora. Como el procesado de las fuentes no ha borrado las columnas no utilizadas en un futuro se podrían extender los scripts de análisis para extraer más características de estas columnas.
...
Se ha utilizado asistencia de IA como apoyo al desarrollo en las siguientes tareas:

\begin{itemize}
  \item Elaboración de las funciones auxiliares \texttt{commas} y
  \texttt{fit} utilizadas en \texttt{awk}, para añadir separadores de
  miles y limitar la longitud de los textos, respectivamente.

  \item Desarrollo del algoritmo de ordenación descendente de los
  sectores según su capitalización bursátil total.

  \item Formateo de los informes \texttt{.txt} mediante tablas de caracteres
  ASCII, alineación de columnas, cabeceras y presentación de
  diferencias de precio y capitalización.

  \item Generación de estructuras HTML para las tablas de los
  informes, definición de estilos CSS e incorporación de numeración
  y totales.

  \item Revisión de fragmentos de Bash y \texttt{awk}, incluyendo
  campos auxiliares de ordenación y condiciones para mostrar
  registros de evolución.

  \item Extensivamente en la generación de tablas y gráficos en el fichero HTML. Generación de Javascript, estilos y parseo de HTML.
